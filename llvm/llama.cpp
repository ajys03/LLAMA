...
struct llamaPass : public FunctionPass {
  ...
  virtual bool runOnFunction(llvm::Function &F) {
    ...
    Instruction *theOp;
		...
		auto &bpi = getAnalysis<llvm::BranchProbabilityInfoWrapperPass>().getBPI();

    // get all frequent BBs
		std::set<BasicBlock*> freq_path;
		std::map<BasicBlock*, float> branch_freq;
    for (auto &B : F) {
      Instruction *tm = B.getTerminator();
      if (BranchInst *bI = dyn_cast<BranchInst>(tm)) {
        BasicBlock *freq = nullptr;
        for (unsigned i = 0; i < bI->getNumSuccessors(); ++i) {
          BasicBlock *successor = bI->getSuccessor(i);
          if (successor) {
            BranchProbability prob = bpi.getEdgeProbability(&B, successor);
            BranchProbability threshold =
                BranchProbability::getBranchProbability(8, 10);
            if (prob >= threshold)
              freq_path.insert(successor);

						float probAsFloat = static_cast<float>(prob.getNumerator()) / prob.getDenominator();
						branch_freq[successor] = probAsFloat;
          }
        }
      }
		}

			// llama score calculation
      for (auto &B : F) {
        ...
        for (auto &I : B) {
          ...
          if (isa<CallInst>(I)) {
            if ((cast<CallInst>(I).getCalledFunction()->getName())
                    .equals("_internal_calloc") ||
                (cast<CallInst>(I).getCalledFunction()->getName())
                    .equals("_internal_malloc") ||
                (cast<CallInst>(I).getCalledFunction()->getName())
                    .equals("_internal_realloc") ||
                (cast<CallInst>(I).getCalledFunction()->getName())
                    .equals("_internal__mm_malloc")) {
              IRBuilder<> builder(op);
              /*  For each allocation via calloc,
               * malloc, realloc, get the insert
               * point, the value (for the store ptr
               * later),  */
              values = addFunc(values, &(cast<Value>(I)), op);
            }
          } /* After a call inst, a store inst is
               performed. */
          if (isa<StoreInst>(I)) {
              ...
              for (auto U : ptrOp->users()) {
                ...
                if (auto *I = dyn_cast<Instruction>(U)) {
                  ...
                  val += pow(std::max(differential, 0), loopDepth * 2);
									...
									// multiply by branch frequency
									val *= branch_freq[&B];
                }
              }

              /*  If we allocated the value
               * referenced by the operand via
               * calloc, malloc, or realloc, count
               * the number of times the pointer
               * operand is used
               */
              values[pos].uses += val;
            }
...
static void
registerllamaPass(const PassManagerBuilder &, legacy::PassManagerBase &PM) {
...
static RegisterStandardPasses
    RegisterMyPass(PassManagerBuilder::EP_EarlyAsPossible, registerllamaPass);

char llamaPass::ID = 0;
static RegisterPass<llamaPass> X("llama", "llama llama", false, false);
