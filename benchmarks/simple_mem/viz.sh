#!/bin/bash
# Usage: viz.sh hw2correctN or viz.sh hw2correctN.fplicm [TYPE]
...
# Choose the correct bitcode file based on the availability of prof data
if [ -f $PROF_DATA ]; then
  BITCODE=$BITCODE_DIR/$BENCH.profdata.bc
else
  BITCODE=$BITCODE_DIR/$BENCH.ls.prof.bc
fi
