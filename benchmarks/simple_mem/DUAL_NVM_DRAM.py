...
params = {
    ...
    "flit_size": "72B",
    "num_ports": rtrInfo.numPorts()
}
memController = sst.Component("router", "merlin.hr_router")
memController.addParams(params)
memController.setSubComponent("topology", "merlin.singlerouter")
...
for x in range(corecount):
    ...
    ArielL1Link.connect((ariel, "cache_link_%d" % x, "60ps"),
                        (l1, "high_network_0", "60ps"))
	"""
    L1L2Link = sst.Link("l1_l2%d" % x)
    L1L2Link.connect((l1, "low_network_0", "60ps"),
                     (l2, "high_network_0", "60ps"))
    L2Link = sst.Link("l2_link_" + str(x))
    L2Link.connect((l2, "directory", "60ps"),
                   (memController, "port" + str(l2_net_addr), "60ps"))
...
link_nvm_bus_link.connect((NVM_controller, "bus", "60ps"),
                          (slow_memory, "cube_link", "60ps"))
...
DRAM_dc_link.connect((DRAM, "direct_link", "60ps"),
                     (DRAM_dc, "memory", "60ps"))
DRAM_net_link = sst.Link("DRAM_dc_net_link")
DRAM_net_link.connect((DRAM_dc, "network", "60ps"),
                      (memController, "port" + str(DRAM_dc_port), "60ps"))

# Connect (slow) NVM to mem controller
slow_dc_link = sst.Link("slow_dc_link")
slow_dc_link.connect((slow_memory, "direct_link", "60ps"),
                     (slow_dc, "memory", "60ps"))  # originally direct_link
slow_net_link = sst.Link("slow_dc_net_link")
slow_net_link.connect((slow_dc, "network", "60ps"),
                      (memController, "port" + str(slow_dc_port), "60ps"))
